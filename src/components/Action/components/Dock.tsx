...
export default function Dock() {
  ...
  return (
    <>
      <nav className="fixed bottom-4 left-1/2 transform-[translate(-50%,0)] flex flex-row gap-2 bg-base-100/50 backdrop-blur-lg rounded-full items-center p-2 hover:opacity-100 justify-center border border-primary">
        <Button variant={InputVariant.Primary}
          className="btn-circle btn-md"
          onClick={handleHTMLClick}
          tooltip={t('dock.code')}
        >
          <IoCode className="size-6" />
        </Button>
        <Button variant={InputVariant.Primary}
          className="btn-circle btn-md"
